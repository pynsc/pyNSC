## NSC
[![Build Status](https://travis-ci.org/uwescience/NSC.svg?branch=master)](https://travis-ci.org/uwescience/NSC)
...
instructions at the [bottom of this page](#using-NSC-as-a-template).
...
`NSC/NSC.py`. The code provided in this file is intentionally rather
...
We place the module code in a file called `NSC.py` in directory called
`NSC`. This structure is a bit confusing at first, but it is a simple way
to create a structure where when we type `import NSC as sb` in an
interactive Python session, the classes and functions defined inside of the
`NSC.py` file are available in the `sb` namespace. For this to work, we
...
In the case of the NSC module, the main classes defined at the bottom of
...
Either way, you can create a `NSC/data` folder in which you can
...
(typically, something like our `NSC/tests/test_NSC.py`). Within each
...
have one such test in `NSC/tests/test_NSC.py` called
...
the top-level directory of the repository (e.g. `/Users/arokem/code/NSC`),
and type:
...
will see a message such as:
    ...
    NSC/tests/test_NSC.py:49: AssertionError
...
To document `NSC` we use the [sphinx documentation
...
![RTD conf](https://github.com/uwescience/NSC/blob/master/doc/_static/RTD-advanced-conf.png)

 http://NSC.readthedocs.org/en/latest/
...
A `NSC/version.py` contains all of the information needed for the
installation and for setting up the [PyPI
page](https://pypi.python.org/pypi/NSC) for the software. This
...
`NSC` against Python 2.7, Python 3.3 and Python 3.4). It includes steps
that need to be taken before installation of the software. For example,
installation of the software dependencies. For `NSC`, we use the
...
[here](https://github.com/uwescience/NSC/blob/master/scripts/Figure1.ipynb).
...
Under "old repository clone URL" enter: https://github.com/uwescience/NSC
...
`NSC/NSC.py` to be called `smallish/smallish.py`

	git mv NSC smallish
	git mv smallish/NSC.py smallish/smallish.py
	git mv smallish/tests/test_NSC.py smallish/tests/test_smallish.py

Make a commit recording these changes. Something like:

	git commit -a -m"Moved names from `NSC` to `smallish`"

You will probably want to remove all the example data:

	git rm smallish/data/*
	git commit -a -m"Removed example `NSC` data"

Possibly, you will want to add some of your own data in there.

You will want to edit a few more places that still have `NSC` in them. Type
the following to see where all these files are:

	git grep NSC

You can replace `NSC` for `smallish` quickly with:

	git grep -l 'NSC' | xargs sed -i 's/NSC/smallish/g'
...
Edit all the mentions of `NSC` in the `NSC/__init__.py` file, and in
the `NSC/version.py` file as well.
